...
$id_actividad = Tools::decrypt(actividad);

$sql ="SELECT 
  cur_doc.* ,
  act_cu.id_tipo_activada
FROM
  actividad_curso AS act_cu 
  JOIN curso_docente AS cur_doc 
    ON cur_doc.curso_doce_id = act_cu.id_curso 
WHERE act_cu.actividad_id = '$id_actividad' ";
...
/*$sql ="SELECT
  estu_mat.*,
  arch_act.archivo,
  arch_act.nombre_archivo,
  arch_act.tipo_archivo,
  nota_act.nota
FROM
  view_estudiantes_matriculados AS estu_mat
  LEFT JOIN archivos_actividad AS arch_act
    ON arch_act.estudiante = estu_mat.estu_id AND arch_act.id_actividad = '$id_actividad'
  LEFT JOIN nota_actividad_estudiante AS nota_act ON  estu_mat.estu_id = nota_act.id_estudiante AND nota_act.id_actividad = '$id_actividad'
...
FROM
  ...
  LEFT JOIN nota_actividad_estudiante AS nota_act ON  estu_mat.estu_id = nota_act.id_estudiante AND nota_act.id_actividad = '$id_actividad'
WHERE  estu_mat.nivel_educativo = '$nivel' AND estu_mat.seccion = '$seccion' AND estu_mat.grado = '$grado'
...
<input type="hidden" value="<?= actividad ?>" id="curso_cod">
                        ...
                        <div class="col-md-12" >
                            ...
                            <table class="table table-striped table-hover">
                                ...
                                <tr>
                                    ...
                                    <?php
                                    if ($contenido['id_tipo_activada']==6){
                                        echo '<th scope="col" class="text-center">Duracion</th>';
                                    }else{
                                        echo '<th scope="col" class="text-center">Archivo</th>';
                                    }
                                    ?>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            ...
                                            if ($contenido['id_tipo_activada']==6){
                                                ...
                                                $sql = "SELECT * FROM alumno_rompecabeza WHERE id_activiidad = '$id_actividad' AND id_alumno= '{$item['estu_id']}'";

                                                if ($row_rompe = $conexion->query($sql)->fetch_assoc()){
                                                    echo '<span class="label label-success">'.$row_rompe['tiempo'].'</span>';
                                                }else{
                                                    echo '<span class="label label-danger">No Resuelto</span>';
                                                }

                                            }else{

                                                $sql="select * from archivos_actividad where id_actividad = '$id_actividad' and estudiante='{$item['estu_id']}'";
                                                $list_resArcg = $conexion->query($sql);
                                                    $isConArch =false;
                                                $tempListArcj=[];
                                                foreach($list_resArcg as $resultA){
                                                    $isConArch =true;
                                                    echo '<a target="_blank" href="'.URL::to('datos/archivos_actividades/'.date('Y').'/'.$contenido['curso_doce_id'].'/'.$resultA['archivo']).'" >'.$resultA['nombre_archivo'].'</a><br>';
                                                }
                                                if (!$isConArch){
                                                    echo '<span class="label label-danger">Sin subir archivo</span>';
                                                }
                                                ?>
