...
if (move_uploaded_file($_FILES['file']['tmp_name'], $location)) {
   ...
   /* if ($row = $archivosActividadAcces->verifica()->fetch_assoc()){
        $archivosActividadAcces->setArchivActividadId($row['archiv_actividad_id']);
        unlink($ruta_princial.$row['archivo']);
        if ($archivosActividadAcces->actualizar()){
            $arr['res']=true;
            $arr['ruta']=$location;
            $arr['nombre']=$filename;
            $arr['tipo']=$path_parts;
