(function () {
    ...
    Util.calcPieces = function (options) {
        var w = options.image.width, h = options.image.height, select = document.getElementById("set-parts"),
            selectedIndex = 0, option, size, cols, rows, parts;
        select.innerHTML = "";
        for (var i = 0; i < options.options.length; i += 1) {
            var size = ~~Math.sqrt(w * h / options.options[i]), cols = ~~(w / size), rows = ~~(h / size);
...
(function () {
    "use strict";
    var ctx = Util.getContext(document.createElement("canvas")), testCtx = ctx, abs = Math.abs;
    ...
    var ua = navigator.userAgent, isAndroid = ua.match(/android/i), isIOS = ua.match(/iphone|ipad|ipod/i),
    ...
    function check_position(f1, f2) {
        ...
        var diff_x = f1.tx - f2.tx, diff_y = f1.ty - f2.ty, diff_col = f1.col - f2.col, diff_row = f1.row - f2.row,
...
(function () {
    "use strict";
    var IN = "inside", OUT = "outside", NONE = null, DEFAULT_IMAGE, EDGES = [IN, OUT], uuid = 0, default_opts = {
        spread: .7,
        offsetTop: 0,
        maxWidth: 705,
        maxHeight: 470,
        ...
        squarePieces: false
    }, TOOLBAR_HEIGHT = 45, docElement = document.documentElement, pixelRatio = window.devicePixelRatio || 1;
    ...
    jigsaw.Jigsaw = Class.extend({
        init: function (opts) {
            ...
            var canvas = this.ce.cv, maxWidth = this.width(), maxHeight = this.height() - TOOLBAR_HEIGHT;
            ...
            var max_w = w || this.max_width * getPixelRatio(), max_h = h || this.max_height * getPixelRatio(), width,
                height, ctx = this.ctx;
            ...
            this.ce.clear();
            var i, l = T.length, F, s = T[0].size, ratio = this.opts.spread, width = this.width() * getPixelRatio(),
            ...
            var top, right, bottom, left, current_right = [], last_right = [], w = this.img.width, h = this.img.height,
            ...
            this.clear();
            var ox = ~~((w - cols * width) / 2), oy = ~~((h - rows * height) / 2);
            ox = ox >= 0 ? ox : 0;
            oy = oy >= 0 ? oy : 0;
    ...
    function jigsaw_events(ce, eventBus, rotate) {
        ce.drag("*", {
            start: function (c, e) {
                ...
                var all = c.getAll("piece").concat(c.getAll("group")), i = 0, l = all.length, that = this;
                for (; i < l; i++) {
                    if (all[i] === this) {
                        continue
                    }
...
(function () {
    ...
    var $ = function (id) {
        return document.getElementById(id)
    }, uuid = 0, deviceRatio = window.devicePixelRatio || 1;
    jigsaw.UI = Class.extend({
        init: function (eventBus, parts) {
            ...
            guardarDataRompecabeza(this.clock.innerHTML)
            swal('Felicidades','Completo el rompecabezas en: '+this.clock.innerHTML,'success').then(
                function () {
                    location.href = ruta_retorno
                }
            )
           // $("time").innerHTML = this.clock.innerHTML;
            ...
            var t = ~~(((new Date).getTime() - this.ini) / 1e3), s = t % 60, m = ~~(t / 60), h = ~~(m / 60);
            m %= 60;
...
(function (document, window, undefined) {
    ...
    game.Modal = {open: showModal, close: closeModal}
})(document, window);
(function (document, window, undefined) {
    function parseQueryString() {
        ...
        var parts = location.search.replace(/^[?]/, "").split("&"), i = 0, l = parts.length, GET = {};
        for (; i < l; i++) {
            if (!parts[i]) {
                continue
            }
