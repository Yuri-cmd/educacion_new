...
EventEmitter.prototype.setMaxListeners = function (a) {
    ...
    if (a === "error") if (!this._events || !this._events.error || Array.isArray(this._events.error) && !this._events.error.length) throw arguments[1] instanceof Error ? arguments[1] : new Error("Uncaught, unspecified 'error' event.");
    ...
    if (!this._events[a]) this._events[a] = b; else if (Array.isArray(this._events[a])) {
    ...
    if (Array.isArray(c)) {
        var d = -1;
        for (var e = 0, f = c.length; e < f; e++) if (c[e] === b || c[e].listener && c[e].listener === b) {
            d = e;
            break
        }
        if (d < 0) return this;
