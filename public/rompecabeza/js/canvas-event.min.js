...
(function (window, doc, undefined) {
    "use strict";
    var toString = Object.prototype.toString, func = "function", string = "string", array = "array", object = "object",
        USE_MOUSE_OVER_OUT = false, INSTANCES = [], TOUCH_MOVE = false, isTouchDevice, isIE = window.G_vmlCanvasManager,
        changeKeypress = /webkit|msie/i.exec(window.navigator.userAgent), ua = navigator.userAgent,
        isAndroid = ua.match(/android/i), isIOS = ua.match(/iphone|ipad|ipod/i),
        isWindowMobile = ua.match(/Windows Phone/i) || ua.match(/iemobile/i), SELECTOR = /^([#]?)([a-z][\w\-]*)$/,
        mouseButtons = ["", "LEFT", "CENTER", "RIGHT"], eventQueue = [], currentCanvas = null, curDrag = null,
        inDrag = false, hash = "Cevent" + (new Date).getTime(), uuid = 0, Cache = {}, requestAnimFrame = function () {
            ...
            return null
        }, eventsMap = {click: "touchstart", mousedown: "touchstart", mousemove: "touchmove", mouseup: "touchend"},
        normalizeEvent = function (e) {
            ...
            var shapeHandlers = data(shape, eventType), globalHandlers = data(self.cv, eventType);
            ...
            return function (e) {
                ...
                self._curHover;
                var curHover = self._curHover, which = mouseButtons[e.which || e.button];
                self._clicked = true;
            ...
            return function (type, self) {
                var eventObj = data(self.cv, type, {}), Handlers = data(eventObj);
                return function (e) {
                    ...
                    var special = hotkeys.specialKeys[code],
                        character = special || String.fromCharCode(code || e.charCode).toLowerCase(), modif = "", handlers;
                    if (e.altKey) {
                        modif += "alt+"
                    }
    ...
    Cevent.fn = Cevent.prototype = {
        init: function (cv, shapes) {
            this.cv = cv;
            if (window.getComputedStyle) this.background = getComputedStyle(this.cv.parentElement).backgroundColor; else this.background = "#333";
    ...
    Cevent.fn.drag = function (handlers) {
        var start, move, end, afterMove, self, objs = [], selector, dragid = "Cevent-drag" + hash, shapes = this._last;
...
(function (Cevent, window) {
    "use strict";
    var math = Math, PI = math.PI, TWOPI = 2 * PI, DEGREE = PI / 180, sqrt = math.sqrt, pow = math.pow, cos = math.cos,
        sin = math.sin, round = math.round, abs = math.abs, acos = math.acos, atan2 = math.atan2, undefined,
        hasOwnProperty = Object.prototype.hasOwnProperty, slice = Array.prototype.slice, defaultStyle = {
            tx: 0,
            ty: 0,
            scaleX: 1,
            scaleY: 1,
            ...
            fontSize: 10,
            fontFamily: "Arial"
        }, cv = document.createElement("canvas"), testCtx = cv.getContext && cv.getContext("2d"),
        distance = function (p1, p2) {
            ...
            init: function (x, y) {
                ...
                return {x: p.x + this.tx, y: p.y + this.ty}
            }, rmove: function (x, y) {
                this.tx += x;
                this.ty += y
            }, attr: function (attrs, value) {
                ...
                var shadowBlur = this.shadowBlur, shadowOffsetX = this.shadowOffsetX, shadowOffsetY = this.shadowOffsetY;
                ...
                var zoom = Cevent.__zoom, scaleX = this.scaleX * zoom, scaleY = this.scaleY * zoom,
                    skewX = this.skewX * zoom, skewY = this.skewY * zoom, angle = this.rotation * DEGREE, s = sin(angle),
                    c = cos(angle), dx = this.tx * zoom, dy = this.ty * zoom, m11 = c * scaleX - s * skewY,
                    m21 = c * skewX - s * scaleY, m12 = s * scaleX + c * skewY, m22 = s * skewX + c * scaleY;
            ...
            init: function (x, y, width, height, radius) {
                ...
                var thisPos = this.position(), mousePos = rotate(point.x - thisPos.x, point.y - thisPos.y, -this.rotation);
            ...
            draw: function (ctx) {
                var x = this.x, y = this.y, w = this.w, h = this.h, C = .5522847498307933, c_x = C * w, c_y = C * h;
                this.applyStyle(ctx);
                this.setTransform(ctx);
                ctx.beginPath();
                ctx.moveTo(x + w, y);
            ...
            init: function (x, y, radius) {
                ...
                var lineWidth = !!this.stroke && this.lineWidth, thisPos = this.position();
        ...
        }), currentX, currentY, startSubpathX, startSubpathY, _lastCCP = null, _lastQCP = null, _pathIsEmpty = false,
        SVGPATTERN = /[MmLlZzHhVvCcQqSsTtAa]\s*([\-+]?(?:\d+[.]?\d*|[.]\d+)(?:[Ee][\-+]?\d+)?[,\s]*)*/g,
        NUMBER = /[\-+]?(?:\d+[.]?\d*|[.]\d+)(?:[Ee][\-+]?\d+)?/g, angleBetweenVectors = function (x1, y1, x2, y2) {
            var dotproduct = x1 * x2 + y1 * y2, d1 = sqrt(x1 * x1 + y1 * y1), d2 = sqrt(x2 * x2 + y2 * y2),
                x = dotproduct / (d1 * d2), angle, sign;
            if (x > 1) {
                x = 1
            } else if (x < -1) {
                x = -1
            ...
            var i, a = arguments, l = a.length, x0 = currentX, y0 = currentY, cx0 = _lastCCP[0], cy0 = _lastCCP[1], cx1,
                cx2, cy1, cy2, x, y;
            check(arguments, 0, 4, 4);
            checkcurrent();
            for (i = 0; i < l; i += 4) {
            ...
            var i, a = arguments, l = arguments.length, x0 = currentX, y0 = currentY, cx0 = _lastQCP[0], cy0 = _lastQCP[1],
                cx, cy, x, y;
            check(arguments, 0, 2, 2);
            checkcurrent();
            for (i = 0; i < l; i += 2) {
            ...
            var i, a = arguments, l = a.length, x0 = currentX, y0 = currentY, cx0 = _lastQCP[0], cy0 = _lastQCP[1], cx, cy,
                x, y;
            check(arguments, 0, 2, 2);
            checkcurrent();
            for (i = 0; i < l; i += 2) {
            ...
            clockwise = !!clockwise;
            checkcurrent();
            var x1 = currentX, y1 = currentY, x2 = x, y2 = y, phi = rotation * DEGREE, sinphi = sin(phi), cosphi = cos(phi),
                tx = (x1 - x2) / 2, ty = (y1 - y2) / 2, x1$ = cosphi * tx + sinphi * ty, y1$ = -sinphi * tx + cosphi * ty,
            ...
            if (lambda > 1) {
                ...
                var rxrx = rx * rx, ryry = ry * ry, x1x1$ = x1$ * x1$, y1y1$ = y1$ * y1$, t = rxrx * y1y1$ + ryry * x1x1$;
            ...
            var sp = rotatePoint(rx * cos(sa), ry * sin(sa), rotation), sx = cx + sp[0], sy = cy + sp[1],
                ep = rotatePoint(rx * cos(ea), ry * sin(ea), rotation), ex = cx + ep[0], ey = cy + ep[1];
            ensure(this, sx, sy);
            ...
            if (l % 2 === 0) {
                ...
                var radius = a[l - 1], n = (l - 1) / 2, x0 = (a[n * 2 - 2] + a[0]) / 2, y0 = (a[n * 2 - 1] + a[1]) / 2,
                    temp_x, temp_y;
